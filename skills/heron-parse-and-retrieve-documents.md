...
operations:
  - POST /end_users
  - POST /end_users/{heron_id}/files/v2
  - POST /end_users/{heron_id}/start_workflow
  - GET /end_users/{end_user_id_or_heron_id}/files
source: >-
  Grounded in Heron's quickstart and API reference (docs.herondata.io). Endpoints
  are the real documented HTTP method+path (no consolidated OpenAPI is published,
  so steps cite paths rather than operationIds).
---
...
2. **Upload documents** — `POST /end_users/{heron_id}/files/v2` with one or more PDFs (bank statements, tax returns, financial statements).
3. **Start processing** — `POST /end_users/{heron_id}/start_workflow` to kick off asynchronous parsing/classification/enrichment.
